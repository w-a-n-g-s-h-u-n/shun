...
class ReactiveRemoteJWKSource implements ReactiveJWKSource {
    ...
    public Mono<List<JWK>> get(JWKSelector jwkSelector) {
        return this.cachedJWKSet.get().switchIfEmpty(getJWKSet()).flatMap(jwkSet -> get(jwkSelector, jwkSet)).switchIfEmpty(getJWKSet().map(jwkSet -> jwkSelector.select(jwkSet)));
    }
